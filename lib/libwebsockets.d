/* 
 * converted from C header to D by bernardh from #d on Freenode (thanks!)
 *
 * All the comments below are from the original libwebsockets.h
...
const short CONTEXT_PORT_NO_LISTEN = 0;

alias int libwebsocket_context_options;
enum : libwebsocket_context_options {
  LWS_SERVER_OPTION_DEFEAT_CLIENT_MASK = 1,
  LWS_SERVER_OPTION_REQUIRE_VALID_OPENSSL_CLIENT_CERT = 2,
}

alias int libwebsocket_callback_reasons;
...
alias int libwebsocket_extension_callback_reasons;
...
alias int libwebsocket_write_protocol;
 ...
 *    internal polling loop, you can just ignore it.
 */
int callback(libwebsocket_context * context,
      libwebsocket *wsi,
...
int extension_callback(libwebsocket_context * context,
      libwebsocket_extension *ext,
      libwebsocket *wsi,
       libwebsocket_callback_reasons reason, void *user,
                void *_in, size_t len);
...
struct libwebsocket_extension {
  ...
  int function(libwebsocket_context *context,
      ...
      libwebsocket_extension_callback_reasons reason,
                void *user, void *_in, size_t len) callback;
  size_t per_session_data_size;
}
...
libwebsocket_create_context(int port, char * interf,
      libwebsocket_protocols *protocols,
      libwebsocket_extension *extensions,
      char *ssl_cert_filepath,
      char *ssl_private_key_filepath, int gid, int uid,
      uint options);

void
libwebsocket_context_destroy(libwebsocket_context *context);

int
libwebsockets_fork_service_loop(libwebsocket_context *context);

int
libwebsocket_service(libwebsocket_context *context, int timeout_ms);

int
libwebsocket_service_fd(libwebsocket_context *context,
               pollfd *pollfd);
...
const short LWS_SEND_BUFFER_PRE_PADDING = 10 + 4;
const short LWS_SEND_BUFFER_POST_PADDING = 1;

int
libwebsocket_write(libwebsocket *wsi, ubyte *buf, size_t len,
             libwebsocket_write_protocol protocol);

int
libwebsockets_serve_http_file(libwebsocket *wsi, char *file,
                 char *content_type);

/* notice - you need the pre- and post- padding allocation for buf below */

int
libwebsockets_broadcast(libwebsocket_protocols *protocol,
            ubyte *buf, size_t len);

libwebsocket_protocols *
libwebsockets_get_protocol(libwebsocket *wsi);

int
libwebsocket_callback_on_writable(libwebsocket_context *context,
                  libwebsocket *wsi);
...
int
libwebsocket_get_socket_fd(libwebsocket *wsi);

int
libwebsocket_is_final_fragment(libwebsocket *wsi);

int
libwebsocket_rx_flow_control(libwebsocket *wsi, int enable);

size_t
libwebsockets_remaining_packet_payload(libwebsocket *wsi);

libwebsocket *
libwebsocket_client_connect(libwebsocket_context *clients,
            char *address,
            int port,
            int ssl_connection,
            char *path,
...
char *
libwebsocket_canonical_hostname(libwebsocket_context *context);
...
void
libwebsockets_hangup_on_client(libwebsocket_context *context, int fd);

void
libwebsocket_close_and_free_session(libwebsocket_context *context,
             libwebsocket *wsi, lws_close_status);

int
libwebsockets_get_random(libwebsocket_context *context,
                  void *buf, int len);

ubyte *
libwebsockets_SHA1(ubyte *d, size_t n, ubyte *md);

libwebsocket_extension libwebsocket_internal_extensions[];
